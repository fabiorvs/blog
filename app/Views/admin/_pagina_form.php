...
<form class="admin-card admin-form-card" action="<?= $action ?>" method="post"><div class="admin-card__body"><?= csrf_field() ?>
    <div class="row g-3 mb-4">
        <div class="col-md-5"><label class="form-label" for="nome">Nome do link</label><input class="form-control" id="nome" name="nome" maxlength="150" value="<?= esc(old('nome', $pagina['nome'] ?? ''), 'attr') ?>" required><div class="form-text">Texto curto exibido no menu do blog.</div></div>
        <div class="col-md-7"><label class="form-label" for="titulo">Título da página</label><input class="form-control" id="titulo" name="titulo" maxlength="200" value="<?= esc(old('titulo', $pagina['titulo'] ?? $pagina['nome'] ?? ''), 'attr') ?>" required><div class="form-text">Título principal exibido ao abrir a página.</div></div>
    </div>
